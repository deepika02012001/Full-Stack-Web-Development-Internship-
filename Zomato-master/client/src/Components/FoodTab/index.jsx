            ...
            <div className="lg:hidden bg-white shadow-lg p-3 fixed bottom-0 z-10 w-full flex items-center justify-between md:justify-evenly text-gray-500 border">
                {
                    allTypes.map((items)=>(
                        <Link to={`/${items.id}`}>
                        <div className={type===items.id?"relative flex flex-col items-center text-xl text-zomato-400":"flex flex-col items-center text-xl"}>
                            <div className={type===items.id && "absolute -top-3 w-full h-full border-t-2 border-zomato-400"} />
                            {items.icon}
...
const LargeTabs = ()=>{
    ...
    return (
        <>
            <div className="hidden lg:flex gap-14 container px-24 mx-auto my-10">
                ...
                {allTypes.map((items)=>(
                    ...
                    <div className={classnames("flex items-center gap-3 pb-2 transition duration-700 ease-in-out",{"border-b-2 border-zomato-400": type===items.id})}>
                        <div className={classnames("w-16 h-16 bg-gray-100 p-4 rounded-full",{[`bg-${items.activeColor}-100`]:type===items.id})}>
                            <img src={type===items.id?items.imageActive:items.imageDefault} alt={items.name} className="w-full h-full" />
                        </div>
                        <h3 className={type===items.id?"text-lg text-zomato-400":"text-xl text-gray-700"}>{items.name}</h3>
                    </div>
                    </Link>
                ))}
            </div>
